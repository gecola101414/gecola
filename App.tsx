...
const App: React.FC = () => {
  ...
  if (view === 'gateway') return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4 font-['Inter']">
      <div className="bg-white rounded-[2.5rem] p-8 md:p-16 shadow-2xl max-w-4xl w-full flex flex-col md:flex-row gap-10 border border-slate-200">
        <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left">
          <EsercitoLogo size="lg" />
          <h1 className="text-3xl md:text-5xl font-black text-slate-800 tracking-tighter uppercase leading-none italic mt-8">
            MANAGEMENT DNA<br/><span className="text-indigo-600">PPB 10.6</span>
          </h1>
        </div>
        <div className="flex-1 flex flex-col gap-3 justify-center">
          ...
          }} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg">CARICA DNA 📂</button>
          <button onClick={() => setView('setup')} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg">NUOVO DNA ➕</button>
        </div>
      </div>
    </div>
  );
  ...
  if (view === 'login' && state) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 font-['Inter']">
      <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl max-w-sm w-full space-y-8 border border-slate-200 text-center">
        <EsercitoLogo size="md" />
        <div className="space-y-4 text-left">
          <select id="l-u" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold appearance-none">
            {state.users.map(u => <option key={u.id} value={u.id}>{u.username}</option>)}
          </select>
          <input type="password" id="l-p" placeholder="Password" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" />
          <button onClick={() => handleLogin((document.getElementById('l-u') as any).value, (document.getElementById('l-p') as any).value)} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest">ACCEDI</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="h-screen w-screen flex flex-col bg-slate-50 font-['Inter'] overflow-hidden">
      ...
      <header className="bg-white px-4 md:px-8 py-4 flex justify-between items-center border-b border-slate-200 z-50 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="h-8 w-8 md:h-10 md:w-10 rounded-full overflow-hidden border-2 border-indigo-100">
            {currentUser.profilePhoto ? <img src={currentUser.profilePhoto} className="w-full h-full object-cover" /> : <div className="bg-slate-200 w-full h-full flex items-center justify-center font-bold">{currentUser.username[0]}</div>}
          </div>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <aside className={`fixed inset-y-0 left-0 w-72 bg-white border-r border-slate-200 z-[110] transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="p-6 h-full flex flex-col">
            ...
            <button onClick={() => window.location.reload()} className="mt-auto p-4 text-[10px] font-black text-rose-600 uppercase border-t border-slate-100 flex items-center gap-3">🚪 Logout</button>
          </div>
        </aside>
      ...
      {(view === 'add-work' || editWorkOrder) && <WorkForm idvs={state.idvs} orders={state.orders} currentUser={currentUser} existingChapters={[]} initialData={editWorkOrder || undefined} onSubmit={(d) => { updateVault({ orders: editWorkOrder ? state.orders.map(o => o.id === editWorkOrder.id ? {...o, ...d} : o) : [...state.orders, {...d as any, id: `w-${Date.now()}`, createdAt: new Date().toISOString()}] }); setEditWorkOrder(null); setView('works'); }} onCancel={() => { setEditWorkOrder(null); setView('works'); }} />}
      {view === 'add-idv' && <IdvForm users={state.users} currentUser={currentUser} existingChapters={[]} onSubmit={(d) => { updateVault({ idvs: [...state.idvs, {...d as any, id: `idv-${Date.now()}`, createdAt: new Date().toISOString()}] }); setView('idvs'); }} onCancel={() => setView('idvs')} />}
