...
import { X, Save, Edit3, Award, TestTubes } from 'lucide-react';
  ...
  useEffect(() => {
    if (isOpen && article) {
      setFormData({
        ...
        soaCategory: article.soaCategory || ''
      });
    }
  }, [isOpen, article]);
          ...
          <form id="edit-article-form" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="col-span-1">
                ...
                <input type="number" step="0.01" value={formData.unitPrice || 0} onChange={(e) => setFormData({...formData, unitPrice: parseFloat(e.target.value)})} className="w-full border border-gray-300 rounded p-2 focus:ring-1 focus:ring-blue-500 outline-none text-right font-mono font-bold text-gray-800" />
              </div>
              ...
              <div className="col-span-2 bg-gray-50 p-2 rounded border border-gray-200">
                ...
                <div className="flex items-center">
                  <input type="range" min="0" max="100" value={formData.laborRate || 0} onChange={(e) => setFormData({...formData, laborRate: parseFloat(e.target.value)})} className="w-full mr-3 accent-blue-600" />
                  <input type="number" value={formData.laborRate || 0} onChange={(e) => setFormData({...formData, laborRate: parseFloat(e.target.value)})} className="w-16 border border-gray-300 rounded p-2 text-right text-sm font-mono" />
                </div>
              </div>
