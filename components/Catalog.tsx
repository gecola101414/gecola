  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500 pb-20 relative overflow-hidden">
      <div className="sticky top-0 z-30 bg-slate-50/90 backdrop-blur-sm py-4 mb-4 border-b border-slate-200 flex justify-between items-center px-4">
         <div className="flex flex-col"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Archivio Pratiche Amministrative</span><span className="text-xs font-bold text-slate-800 uppercase">{sortedOrders.length} Fascicoli in Registro</span></div>
      </div>
          ...
          return (
            <div key={o.id} ref={el => { refs.current[o.id] = el; }} className={`bg-white rounded-[1.8rem] p-4 border-2 shadow-sm transition-all flex items-center gap-6 group relative ${o.locked ? 'opacity-80 border-slate-100' : 'border-indigo-50 hover:shadow-lg'} ${highlightId === o.id ? 'border-indigo-500 ring-8 ring-indigo-500/10 scale-[1.01] z-20' : ''}`}>
              ...
              <div className="w-56 flex flex-col gap-1">
                <div className="flex items-center gap-2"><span className="bg-slate-900 text-white text-[8px] font-black px-2 py-0.5 rounded uppercase">{o.orderNumber}</span><button onClick={() => onChapterClick(chapter)} className={`px-2 py-0.5 rounded bg-${color}-50 text-${color}-600 border border-${color}-100 text-[8px] font-black uppercase`}>Cap. {chapter}</button></div>
              ...
              <div className="flex items-center gap-2 ml-auto">
                <button onClick={() => onShowHistory(o.id)} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-900 transition-all" title="Storico Pratica">📜</button>
