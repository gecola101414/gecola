...
import { Layers, Award, CheckCircle2, AlertTriangle, Calculator, FileText, ShieldCheck } from 'lucide-react';
...
const Summary: React.FC<SummaryProps> = ({ totals, info, categories, articles }) => {
  ...
  // Calcoli finali (Lavori + Sicurezza Analitica)
  const totalLavoriSolo = wbsBreakdown
    .filter(cat => cat.code !== 'WBS.SIC')
    .reduce((sum, cat) => sum + cat.total, 0);

  const safetyAnalitico = wbsBreakdown
    .find(cat => cat.code === 'WBS.SIC')?.total || 0;

  const grandTotal = totalLavoriSolo + safetyAnalitico; // SENZA IVA
              ...
              <div className="bg-white shadow-sm border border-gray-200 rounded-lg overflow-hidden flex-1">
                  <table className="w-full text-sm">
                      ...
                      <tfoot className="bg-blue-50 border-t-2 border-blue-100">
                          <tr>
                              <td colSpan={2} className="p-3 text-right font-bold text-blue-900 uppercase text-xs">Importo Complessivo Lavori + Sicurezza</td>
      ...
      <div className="bg-white p-8 shadow-lg rounded-xl border border-gray-300 mt-8">
          ...
          <div className="space-y-4 text-sm max-w-2xl ml-auto">
              <div className="flex justify-between items-center text-gray-600 text-base border-b border-gray-100 pb-2">
                  <span>Importo Lavori a misura e a corpo (A)</span>
                  <span className="font-mono font-black text-slate-800">{formatCurrency(totalLavoriSolo)}</span>
              </div>
              <div className="flex justify-between items-center text-gray-600 text-base border-b border-gray-100 pb-2">
              ...
              <div className="pt-4 flex justify-between items-center bg-blue-50 p-6 rounded-lg border border-blue-100">
                  <div className="flex flex-col">
                      <span className="font-bold text-blue-900 uppercase text-xl tracking-tight">Totale Appalto</span>
                      <span className="text-blue-500 text-xs uppercase tracking-widest">(A + B)</span>
                  </div>
                  <span className="font-mono text-4xl font-black text-blue-700">{formatCurrency(grandTotal)}</span>
              </div>
          </div>
      </div>
              ...
              <div>
                  ...
                  <p className={`text-xs mt-1 ${isBalanced ? 'text-green-600' : 'text-red-600'}`}>
                      {isBalanced 
                          ...
                          : `Esiste una differenza di ${formatCurrency(Math.abs(totalWbsAmount - totalSoaAmount))} tra il totale WBS e il totale SOA.`}
                  </p>
              </div>
          </div>
      </div>
