...
const ArticleGroup: React.FC<any> = (props) => {
   const { article, index, allArticles, isPrintMode, isCategoryLocked, onUpdateArticle, onEditArticleDetails, onDeleteArticle, onAddMeasurement, onAddSubtotal, onUpdateMeasurement, onDeleteMeasurement, onToggleDeduction, onOpenLinkModal, onScrollToArticle, onArticleDragStart, onViewAnalysis, onToggleArticleLock } = props;
      ...
      <tbody id={`article-${article.id}`} className={`bg-white border-b border-gray-400 relative ${isArticleLocked ? 'bg-gray-50' : ''}`}>
         ...
         {processedMeasurements.map((m: any) => (
            <tr key={m.id} className={`text-xs group/meas transition-colors ${m.type === 'deduction' ? 'text-red-600' : 'text-gray-800'} ${m.type === 'subtotal' ? 'bg-yellow-50 font-bold' : 'bg-white hover:bg-slate-50'}`}>
                ...
                <td className="border-r border-gray-200 p-0 bg-gray-50"><input type="number" disabled={areControlsDisabled} className="w-full text-center bg-transparent border-none text-xs" value={m.multiplier || ''} placeholder="1" onChange={(e) => onUpdateMeasurement(article.id, m.id, 'multiplier', parseFloat(e.target.value))} /></td>
                <td className="border-r border-gray-200 p-0 bg-gray-50"><input type="number" disabled={areControlsDisabled || !!m.linkedArticleId} className="w-full text-center bg-transparent border-none text-xs" value={m.length || ''} onChange={(e) => onUpdateMeasurement(article.id, m.id, 'length', parseFloat(e.target.value))} /></td>
                <td className="border-r border-gray-200 p-0 bg-gray-50"><input type="number" disabled={areControlsDisabled || !!m.linkedArticleId} className="w-full text-center bg-transparent border-none text-xs" value={m.width || ''} onChange={(e) => onUpdateMeasurement(article.id, m.id, 'width', parseFloat(e.target.value))} /></td>
                <td className="border-r border-gray-200 p-0 bg-gray-50"><input type="number" disabled={areControlsDisabled || !!m.linkedArticleId} className="w-full text-center bg-transparent border-none text-xs" value={m.height || ''} onChange={(e) => onUpdateMeasurement(article.id, m.id, 'height', parseFloat(e.target.value))} /></td>
...
const App: React.FC = () => {
  ...
  /**
   * BLOCCO MAGICO 1: BYPASS CURSORE GLOBALE
   * Forza il browser ad accettare il drop ed evita il simbolo del divieto (prohibited)
   */
  useEffect(() => {
  ...
  /**
   * BLOCCO MAGICO 2: ATTIVATORE DI TAB (DRAG START)
   * Aggiunge un finto URL che obbliga Chrome a cambiare scheda durante il trascinamento
   */
  ...
  /**
   * BLOCCO MAGICO 3: LOGICA DROP CROSS-TAB
   * Riceve il bundle JSON, rigenera tutti gli ID e lo inserisce nel nuovo file
   */
  ...
  return (
    <div className="h-screen flex flex-col bg-[#e8eaed] font-sans overflow-hidden text-slate-800">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar Navigazione */}
        <div className="w-64 bg-white border-r border-slate-300 flex flex-col z-10 shadow-lg" onDragOver={(e) => { e.preventDefault(); e.dataTransfer.dropEffect = 'copy'; }} onDrop={(e) => handleWbsDrop(e, null)}>
          ...
          <div className="flex-1 overflow-y-auto">
            {viewMode === 'COMPUTO' ? (
                ...
                <div className="p-2 space-y-2">
                    {analyses.map(an => (
                        <div key={an.id} draggable onDragStart={(e) => { e.dataTransfer.setData('text/plain', `ANALYSIS_BUNDLE::${JSON.stringify(an)}`); e.dataTransfer.effectAllowed='copy'; }} className="bg-white p-3 rounded border border-slate-200 shadow-sm hover:border-purple-300 transition-all cursor-grab group">
                ...
                <div className="flex flex-col h-full">
                  <div className="p-4 bg-gray-50 border-b flex items-center justify-between sticky top-0 z-30 shadow-sm">
                      ...
                      <div className="flex items-center gap-4 w-[450px]">
                          <div className="flex-1"><CategoryDropGate onDropContent={(txt: string) => { const p = parseDroppedContent(txt); if(p) updateState([...articles, { ...p, id: Math.random().toString(36).substr(2,9), categoryCode: activeCategory.code, measurements: [{id: Math.random().toString(36).substr(2,9), description: '', type: 'positive'}] } as Article]) }} isLoading={false} categoryCode={activeCategory.code} /></div>
                  ...
                  <div className="flex-1 overflow-y-auto">
                      <table className="w-full text-left border-collapse min-w-[1000px]">
                          ...
                          {activeArticles.map((art: any, idx: number) => (
